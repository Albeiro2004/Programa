...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!$cantidadValida) {
        ...
        window.location.href = 'Facturar1.php?mensaje=Factura creada con éxito!';
        </script>";
...
<head>
    ...
    <title>Facturar sin Cliente</title>
...
<?php if ($mensaje): ?>
    ...
    <div id="total-container" style="position: fixed; top: 20px; right: 150px; background-color: #f0f0f0; padding: 10px; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.1);">
    ...
    <button id="boton-fijo" onclick="window.open('infoFacturas.php', '_blank')">
    <i class="fas fa-file-invoice"></i> Ir a Facturas
    </button>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let inputsCantidad = document.querySelectorAll("input[name^='productos']");

    inputsCantidad.forEach(input => {
        input.addEventListener("input", function () {
            let fila = this.closest("tr");  // Obtener la fila del producto
            let disponible = parseInt(fila.querySelector("td:nth-child(5)").textContent);  // Cantidad disponible
            
            // Verificar si la cantidad ingresada supera la disponible
            if (parseInt(this.value) > disponible) {
                alert("La cantidad seleccionada no puede ser mayor a la disponible.");
                this.value = disponible;  // Ajustar al máximo disponible
            }
        });
    });
});
...
#boton-facturar:focus {
   ...
   /* Estilo para fijar el botón en la parte inferior izquierda */
   #boton-fijo {
            position: fixed;
            top: 20px;  /* Espacio desde la parte inferior */
            left: 10px;    /* Espacio desde la parte izquierda */
            padding: 10px 10px;
            background-color:rgba(98, 142, 192, 0.97);
            color: white;
            border: none;
            border-radius: 5px;
            font-size: 16px;
